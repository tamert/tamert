...
# Token used for GitHub GraphQL API calls.
# - Prefer a fine-grained PAT in ACCESS_TOKEN if you want to include private repos / org repos.
# - Falls back to GITHUB_TOKEN when running in GitHub Actions (public data may be limited).
_TOKEN = os.environ.get('ACCESS_TOKEN') or os.environ.get('GITHUB_TOKEN')
if not _TOKEN:
    raise RuntimeError("Missing token: set ACCESS_TOKEN (PAT) or GITHUB_TOKEN")

HEADERS = {'authorization': 'token ' + _TOKEN}
USER_NAME = os.environ.get('USER_NAME') or 'tamert'
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    # Profile header + optional fields.
    profile_title = os.environ.get('PROFILE_TITLE') or f"{USER_NAME}@github"
    github_value = f"github.com/{USER_NAME}"
    find_and_replace(root, 'profile_title', _truncate_text(profile_title, 18))
    ...
    for element_id, (value, max_len) in optional_fields.items():
        if value:
            find_and_replace(root, element_id, _truncate_text(value, max_len))
...
if __name__ == '__main__':
    ...
    created_at = datetime.datetime.fromisoformat(acc_date.replace('Z', '+00:00')).replace(tzinfo=None)
    ...
    # Optional: add back archived repository contributions (if you maintain cache/repository_archive.txt)
    if os.environ.get('USE_REPO_ARCHIVE') == '1':
